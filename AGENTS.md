...
- **No code scan was possible here.** The scanner is a Python `ast` pass over `os.environ` literals, and this repo has no Python sources — reads through any other mechanism (`process.env` in JS/TS, `os.Getenv` in Go) are invisible to it. The env var list therefore reflects the Render blueprint alone.
